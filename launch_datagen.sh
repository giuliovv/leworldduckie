...
DT_WORLD=\$(python3 -c "import duckietown_world,os;print(os.path.dirname(duckietown_world.__file__))" 2>/dev/null)
DT_GYM=\$(python3 -c "import gym_duckietown,os;print(os.path.dirname(gym_duckietown.__file__))" 2>/dev/null)
ln -sf "\${DT_WORLD}/data/gd1/maps" "\${DT_GYM}/maps" 2>/dev/null || echo "symlink skipped"
echo "symlink done"
...
cd /tmp
DISPLAY=:99 python3 generate_data.py --n-transitions ${N_TRANSITIONS} --out /tmp/duckietown_100k.h5
EXIT_CODE=\$?
echo "python exit \${EXIT_CODE}"

if [ \${EXIT_CODE} -eq 0 ]; then
    aws s3 cp /tmp/duckietown_100k.h5 s3://${DATA_BUCKET}/data/duckietown_100k.h5 --region ${REGION} || \
    aws s3 cp /tmp/duckietown_100k.h5 s3://${LOG_BUCKET}/lewm-training/datagen/duckietown_100k.h5 --region ${REGION} || true
fi

aws s3 cp "\$LOG" s3://${LOG_BUCKET}/lewm-training/datagen-logs/datagen_${RUN_ID}.log --region ${REGION} || true
...
echo ""
echo "Log (after instance finishes ~5 min bootstrap + 2.5h run):"
echo "  aws s3 cp s3://${LOG_BUCKET}/lewm-training/datagen-logs/datagen_${RUN_ID}.log -"
echo ""
